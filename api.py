    ...
    try:
        ...
        for frame_idx, frame in frame_generator(stream_url):
            ...
            while frame_queue.full():
                # Queue full, drop oldest frame item if exists
                if not frame_queue.empty():
                    item = frame_queue.get()
                    if item[0] != 'frame':
                        # Put back non-frame items (info, end)
                        frame_queue.put(item, block=False)
                    # If it's a frame, we drop it (already removed from queue)
                else:
                    time.sleep(0.01)
...
def frame_processor_process(stream_url: str, frame_queue: multiprocessing.Queue,
    ...
    # Get video info from queue
    item = frame_queue.get()
    if item[0] != 'info':
        print(f"[Task {task_id}] Processor: Error - expected info, got {item[0]}")
...
def stream_writer_process(processed_queue: multiprocessing.Queue,
    ...
    item = processed_queue.get()
    if item[0] != 'info':
        print(f"[Task {task_id}] Streamer: Error - expected info, got {item[0]}")
        return
...
def process_video_streams(stream_urls: List[str], output_stream_url: str,
    ...
    frame_queue = multiprocessing.Queue(maxsize=10)  # Raw frames
    processed_queue = multiprocessing.Queue(maxsize=10)  # Processed frames
